...
async fn main() {
    ...
    let (tx, mut msg_rx, h_ser_read, h_ser_write) =
        esp_io::connect_serial(cfg.serial_port.clone(), cfg.output_dir.clone()).await;
    ...
    if let Some((server_ip, server_port)) = cfg.server.clone() {
        ...
        let h = tokio::spawn(async move {
            if let Err(e) =
                connect_and_forward_to_server(server_ip, server_port, msg_rx, cmd_tx).await
            {
        ...
        let h = tokio::spawn(async move { while let Some(_) = msg_rx.recv().await {} });
        handles.push(h);
    }
